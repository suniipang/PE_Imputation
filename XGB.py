...
for train, test in logo.split(Ximp_scaled, Yimp, groups=wells_noPE):
    ...
    # Imputation using XGBRegressor
    xgb_model = xgboost.XGBRegressor(n_estimators=500, max_depth=6, eta=0.2)
    xgb_model.fit(Ximp_scaled[train], Yimp[train])
    # xgboost.plot_importance(xgb_model)

    # R2 = xgb_model.score(Ximp_scaled[test],Yimp[test]) # R2
    # print("Well name_test : ", well_name)
    # print("R2: %.4f" %R2)

    Yimp_predicted = xgb_model.predict(Ximp_scaled[test])
